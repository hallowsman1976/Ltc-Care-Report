...
# 9. REST API + Frontend บน GitHub Pages

ระบบรองรับ **2 โหมดการ host frontend** โดยใช้โค้ดชุดเดียวกัน:

| โหมด | host ที่ไหน | frontend เรียก backend ผ่าน |
|------|-----------|---------------------------|
| **A. GAS-hosted** (เดิม) | Web App URL ของ GAS เอง (`/exec`) | `google.script.run` (RPC ภายใน) |
| **B. GitHub Pages** | `https://<user>.github.io/<repo>/` | `fetch` → GAS REST API |

`rpc()` ใน `script.html` เป็น **dual-mode** — ถ้ามี `window.LTC_API_URL` (ตั้งใน `docs/config.js`) จะใช้ REST, ไม่งั้นใช้ `google.script.run` อัตโนมัติ

## 9.1 สถาปัตยกรรม REST

```
External client / GitHub Pages frontend
        │  POST <WEBAPP_URL>/exec   (Content-Type: text/plain เพื่อเลี่ยง CORS preflight)
        │  body: { "action":"invoke", "fn":"api_login", "args":["admin","admin123"] }
        ▼
RestApi.gs
  ├─ doPost(e) / doGet(e?action=)  → _restDispatch_()
  ├─ REST_ROUTES[action]           → 74 named routes (login, getPatients, ...)
  └─ action 'invoke'               → proxy เรียก api_* ตามชื่อ (allowlist) เลียนแบบ google.script.run
        ▼
  api_*() → Service layer → Google Sheets
```

**Endpoint รูปแบบเรียกได้ 2 แบบ:**
- **Named route:** `{ "action":"login", "payload":{ "username":"admin", "password":"admin123" } }`
- **Generic invoke:** `{ "action":"invoke", "fn":"api_login", "args":["admin","admin123"] }` ← frontend ใช้แบบนี้

ทุก response = `{ success, data, message }` — client เช็คจาก field `success` (GAS ตั้ง HTTP status code จริงไม่ได้)

> **CORS:** ต้องส่ง `Content-Type: text/plain;charset=utf-8` เท่านั้น (simple request) ถ้าใช้ `application/json` browser จะยิง preflight OPTIONS ที่ GAS ตอบไม่ได้ → ล้มเหลว response ของ GAS มี `Access-Control-Allow-Origin: *` ให้อยู่แล้ว

## 9.2 ครั้งแรก — ติดตั้ง clasp + npm scripts

```bash
npm install -g @google/clasp   # ถ้ายังไม่มี
clasp login                    # ครั้งเดียว
```

`package.json` มี scripts ช่วย workflow:

| คำสั่ง | ทำอะไร |
|--------|--------|
| `npm run push` | `clasp push --force` — อัปโค้ด `.gs`/`.html` ขึ้น GAS |
| `npm run deploy:gas` | สร้าง version ใหม่ของ Web App deployment (URL `/exec` ถึงจะเห็นโค้ดใหม่) |
| `npm run backend` | `push` + `deploy:gas` รวบเดียว (แก้ backend แล้วใช้ตัวนี้) |
| `npm run build` | `node build-web.js` — inline partials → `docs/index.html` |
| `npm run pages` | `build` + commit `docs/` + push GitHub |
| `npm run release` | `push` + `deploy:gas` + `build` — release ครบทุกอย่าง |

> ค่า deployment id ใน `deploy:gas` ฝังไว้ใน `package.json` แล้ว ถ้าเปลี่ยน deployment ต้องแก้ตรงนั้น

## 9.3 Workflow หลังแก้โค้ด

**แก้ backend (`.gs`):**
```bash
npm run backend          # push + deploy เวอร์ชันใหม่
```

**แก้ frontend (`.html` partials เช่น case-form.html):**
```bash
npm run backend          # (ถ้าโหมด A ยังใช้อยู่) อัป GAS ด้วย
npm run pages            # rebuild docs/ + push GitHub → Pages อัปเองใน ~1 นาที
```

> ⚠️ **กฎเหล็ก:** `clasp push` แค่อัปโค้ด — URL `/exec` ยังรันเวอร์ชันเก่าจนกว่าจะ **deploy version ใหม่** (`npm run deploy:gas`) นี่คือสาเหตุยอดฮิตของอาการ "แก้แล้วไม่เปลี่ยน"

## 9.4 เปิด GitHub Pages (ครั้งเดียว)

1. ใส่ URL ของ Web App ใน `docs/config.js`:
   ```js
   window.LTC_API_URL = 'https://script.google.com/macros/s/AKfyc.../exec';
   ```
2. push: `git add docs/config.js && git commit -m "set api url" && git push`
3. GitHub repo → **Settings → Pages → Source: Deploy from a branch → Branch `main` + folder `/docs` → Save**
4. รอ ~1 นาที → เปิด `https://<user>.github.io/<repo>/` → login

## 9.5 ความปลอดภัย REST (แนะนำ)

- **API Key gate:** ตั้ง Setting key `REST_API_KEY` ในชีต `Setting` → endpoint สาธารณะ (public) จะบังคับแนบ `apiKey` ตรงกัน (ใส่ใน `docs/config.js` → `window.LTC_API_KEY`)
- **ล็อก `setupSheets`:** หลังติดตั้งเสร็จ แก้ route `setupSheets` ใน `RestApi.gs` เป็น `public:false` หรือลบทิ้ง กันคนนอกเรียก reset
- **เปลี่ยนรหัส admin:** ระบบ public บนเน็ตแล้ว — ต้องเปลี่ยนจาก `admin123` ทันที (ดู Checklist ข้อ 6)

## 9.6 ทดสอบ REST เร็วๆ

```bash
# GET (ง่ายสุด)
curl -sL "<WEBAPP_URL>/exec?action=ping"

# POST invoke (เลียนแบบ frontend) — บน Windows ให้ follow redirect เป็น GET เอง
#   เบราว์เซอร์ทำให้อัตโนมัติ ไม่ต้องกังวล
curl -s -X POST "<WEBAPP_URL>/exec" \
  -H "Content-Type: text/plain;charset=utf-8" \
  --data-binary '{"action":"invoke","fn":"api_ping","args":[]}'
```
